...
$pendingfund = $obj->selectfieldwhere("fundrequest inner join users on users.id = fundrequest.userid", "count(fundrequest.id)", "fundrequest.status =0 $empref");
$todayaitraders = $obj->selectfieldwhere("aitraders inner join users on users.id = aitraders.userid", "count(aitraders.id)", "aitraders.status =1 $empref");
?>
