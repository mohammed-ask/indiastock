...
$return['recordsTotal'] = $obj->selectfieldwhere("stocktransaction $join", "count(stocktransaction.id)", "stocktransaction.status = 0 and (date(stocktransaction.added_on) = date(CONVERT_TZ(NOW(),'+00:00','$timeskip')) || date(stocktransaction.datetime) = date(CONVERT_TZ(NOW(),'+00:00','$timeskip')))");
$return['recordsFiltered'] = $obj->selectfieldwhere("stocktransaction $join", "count(stocktransaction.id)", "stocktransaction.status = 0 and (date(stocktransaction.added_on) = date(CONVERT_TZ(NOW(),'+00:00','$timeskip')) || date(stocktransaction.datetime) = date(CONVERT_TZ(NOW(),'+00:00','$timeskip'))) $search ");
$return['draw'] = $_GET['draw'];
$result = $obj->selectextrawhereupdate(
    ...
    "stocktransaction.status = 0 and (date(stocktransaction.added_on) = date(CONVERT_TZ(NOW(),'+00:00','$timeskip')) || date(stocktransaction.datetime) = date(CONVERT_TZ(NOW(),'+00:00','$timeskip'))) $search $order limit $start, $limit"
);
